...
def create_report(patient_name, sessions):
    ...
    # הפרומפט המקצועי המקורי שלך - ללא שינויים!
    prompt = f"""אתה קלינאי תקשורת מומחה בכיר. עליך להפוך את ההערות הגולמיות לדוח "בקשת המשך טיפול" רשמי ומקצועי.
...
def revise_report_with_history(patient_name, history, new_instructions):
    # בניית הקשר ההיסטוריה לתיקון
    context = ""
    for msg in history:
        prefix = "קלינאית" if msg.role == "user" else "AI"
        context += f"{prefix}: {msg.content}\n"
    
    # הנחיה לתיקון ששומרת על הסטנדרט המקצועי המקורי
    revision_prompt = f"""
    אתה קלינאי תקשורת מומחה בכיר. עליך לעדכן דוח קיים על סמך בקשת המשתמש, תוך שמירה קפדנית על הרמה הקלינית הגבוהה.
    
    היסטוריית הדוח:
    {context}
    
    הוראת התיקון החדשה: "{new_instructions}"
    
    דרישות לתיקון:
    1. שמור על המבנה המקורי (בס"ד, כותרות, פרטי מטופל).
    2. השתמש במושגים מקצועיים: מורפולוגיה, תחביר, מובנות דיבור, ארגון מסר.
    3. בצע את השינוי המבוקש עבור המטופל {patient_name} בצורה אינטגרלית בתוך הדוח.
    """
    return generate_with_gemini(revision_prompt)